    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.gongzuoleixing != '') searchWhere.gongzuoleixing = '%' + this.formSearch.gongzuoleixing + '%';
	      params['sfsh'] = '是';
        this.$http.get('toujianxinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/toujianxinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
